...
AS (
	...
	SELECT p.person_id
		...
		,DATEDIFF(yy, COALESCE(vd.visit_detail_start_datetime, vd.visit_detail_start_date, vo.visit_start_datetime, vo.visit_start_date), COALESCE(p.birth_datetime, DATEFROMPARTS(p.year_of_birth, COALESCE(p.month_of_birth, '06'), COALESCE(p.day_of_birth, '01')))) AS age
	...
	LEFT JOIN {schema}.visit_detail vd ON p.person_id = vd.person_id
	-- gender concept
...
SELECT adm.*
	,DATEDIFF(dd, COALESCE(m.measurement_datetime, m.measurement_date), adm.icu_admission_datetime) AS days_in_icu
...
LEFT JOIN {schema}.measurement m
	...
	AND (
		adm.visit_detail_id = m.visit_detail_id
		OR adm.visit_detail_id IS NULL
		)
	AND DATEDIFF(dd, COALESCE(m.measurement_datetime, m.measurement_date), adm.icu_admission_datetime) >= {min_day}
	AND DATEDIFF(dd, COALESCE(m.measurement_datetime, m.measurement_date), adm.icu_admission_datetime) < {max_day}
...
GROUP BY adm.person_id
	...
	,DATEDIFF(dd, COALESCE(m.measurement_datetime, m.measurement_date), adm.icu_admission_datetime)
