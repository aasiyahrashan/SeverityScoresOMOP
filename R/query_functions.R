...
#' @param start_date The earliest ICU admission date/datetime.
#'   Needs to be in character format.
#' @param end_date As above, but for last date
#' @param min_day The number of days since ICU admission to get physiology data
#'   for. Starts with 0
#' @param max_day The number of days since ICU admision to get physiology data
#'   for.
#' @param mapping_path Path to *_concepts.csv file containing score to OMOP
#'   mappings. Should match the example_concepts.csv file format.
#' @param severity_score The name of the severity score to calculate. Only
#'   APACHE II at the moment.
#'
#' @import lubridate
#' @import DBI
#' @import dplyr
...
get_score_variables <- function(conn, driver, schema, start_date, end_date,
                                min_day, max_day, concepts_file_path,
                                severity_score) {

  #### Getting the list of concept IDs required for the score, and creating SQL
  #### lines from them.
    ...
    mutate(
      max_query = glue(
        ", MAX(CASE WHEN m.measurement_concept_id = {concept_id}
          THEN m.{omop_variable} END) AS max_{short_name}"
      ),
      min_query = glue(
        ", MIN(CASE WHEN m.measurement_concept_id = {concept_id}
          THEN m.{omop_variable} END) AS min_{short_name}"
      ),
      unit_query = glue(
        ", MIN(CASE WHEN m.measurement_concept_id = {concept_id}
          THEN c_unit.concept_name END) AS unit_{short_name}"
      )
    )
  ...
  variables_required <-
    glue(
      glue_collapse(measurement_concepts$max_query, sep = "\n"),
      "\n",
  ...
  if (grep("SQL Server", driver, ignore.case = TRUE)) {
    file_name_physiology_variables <-
      "physiology_variables_sql_server.sql"
  } else{
    file_name_physiology_variables <- "physiology_variables.sql"
  }
  raw_sql <-
    readr::read_file(system.file(file_name_physiology_variables,
  ...
  gcs_concepts <- concepts %>%
    filter((
      short_name %in% c("gcs_eye", "gcs_motor", "gcs_verbal") &
        omop_variable == "value_as_concept_id"
    ))
